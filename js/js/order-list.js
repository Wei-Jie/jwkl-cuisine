...
const OrderList = (() => {
    ...
    function renderSummaryTable() {
        ...
        pendingSchedules.forEach(s => {
            // 由於 rowsToObjects 可能因為試算表標題不同而產生不同 key，這裡使用防禦性匹配
            const name = s['品項'] || s['品項名稱'] || '';
            const rawQty = s['數量'] || s['訂購數量'] || 0;
        ...
        try {
            ...
            for (const cb of checked) {
                ...
                if (order._computedStatus !== '已完成' && order._computedStatus !== '已出貨' && newStatus === '已完成' && order['Email']) {
                    const itemsText = scheduleLines.map(s => `✔️ ${s['品項'] || s['品項名稱'] || ''} x${s['數量'] || s['訂購數量'] || 0}`).join('<br>');
        ...
        try {
            if (remainingItemsCount === 0) {
                ...
                if (!wasCompleted && isNowCompleted && order['Email']) {
                    const itemsText = detailLines.filter(l => !l._deleted).map(l => `✔️ ${l['品項名稱'] || l['品項'] || ''} x${l['數量'] || l['訂購數量'] || 0}`).join('<br>');
