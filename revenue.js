            ...
            lines.forEach(l => {
                ...
                if (!itemStats[name]) {
                    ...
                    if (menuItem) {
                        ...
                        if (costStr.includes('%')) {
                            const pct = parseFloat(costStr) / 100;
                            const priceStr = String(getVal(menuItem, ['單價']) || '0');
                            const price = parseFloat(priceStr.replace(/[^0-9.]/g, '')) || 0;
                            cost = Math.round(price * pct);
                        } else {
                            cost = parseFloat(costStr.replace(/[^0-9.]/g, '')) || 0;
                        }
                    }
                    
                    itemStats[name] = { qty: 0, amount: 0, category: menuItem?.['分類'] || '無/舊品項', unitCost: cost };
                }
